...
import WebGL.Texture as Texture exposing (Error, Texture, FrameBuffer)
...
view : Model -> Html Msg
view { pass, resolution, theta, texture } =
        ...
        ( Just frameBuffer, Just tex ) ->
            WebGL.toHtmlWith
                ...
                [ WebGL.entity
                    ...
                    passMesh
                    (BlurUniforms
                        (Texture.fromEntities frameBuffer (scene (perspective theta) tex))
                        resolution
                    )
                ]
...
type alias BlurUniforms =
    { pass : Texture
    , resolution : Vec2
    }


passVertex : Shader { position : Vec2 } BlurUniforms {}
passVertex =
    [glsl|
...
passFragment : Shader {} BlurUniforms {}
passFragment =
    [glsl|
        ...
        void main () {
            ...
            for (int j = 0; j <= kSize; ++j) {
                float x = float(j);
                kernel[kSize+j] = kernel[kSize-j] = 0.39894 * exp((-0.5 * x * x) / (sigma * sigma)) / sigma;
            }
            ...
            for (int i = -kSize; i <= kSize; ++i) {
                for (int j = -kSize; j <= kSize; ++j) {
                    vec2 uv = gl_FragCoord.xy + vec2(float(i), float(j));
                    final_colour += kernel[kSize+j] * kernel[kSize+i] * texture2D(pass, uv / resolution);
                }
            }
